...
dataset = np.genfromtxt("datasets/data_banknote.csv" , delimiter=',')
#ap = Imbalance(datasets)
#ap.calcutate()
#ap.plot()
#X = dataset[:, :-1]
#y = dataset[:, -1].astype(int)
X = np.genfromtxt("datasets/Mirai_dataset.csv", delimiter=',')
y = np.genfromtxt("datasets/Mirai_labels.csv").astype(int)
...
for fold_id, (train, test) in enumerate(rskf.split(X, y)):
    for preproc_id, preproc_name in enumerate(preprocs):
        ...
        scores_GNB[preproc_id, fold_id] = accuracy_score(y[test],y_pred)

        #SVC
        clf_SVC = SVC()
        clf_SVC.fit(X_res, y_res)
        y_pred = clf_SVC.predict(X[test])
        scores_SVC[preproc_id, fold_id] = accuracy_score(y[test],y_pred)

        #kNN
        clf_kNN = KNeighborsClassifier()
        clf_kNN.fit(X_res, y_res)
        y_pred = clf_kNN.predict(X[test])
        scores_kNN[preproc_id, fold_id] = accuracy_score(y[test],y_pred)
            
        #LSVC
        clf_LSVC = LinearSVC(random_state=1234, tol=1e-5)
        clf_LSVC.fit(X_res, y_res)
        y_pred = clf_LSVC.predict(X[test])
        scores_LSVC[preproc_id, fold_id] = accuracy_score(y[test],y_pred)
